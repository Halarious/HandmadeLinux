...
#undef _mmSquare 
#define _mmSquare(a) ++Count.mm_mul_ps; a
#define __m128  s32
#define __m128i s32
	  ...
	  //if(_mm_movemask_epi8(WriteMask))
		  ...
		  ++I)
		{
		  ...
		  u8* TexelPtr = (((u8*)Texture->Memory)
				  ...
				  + FetchX * BITMAP_BYTES_PER_PIXEL);
		  Mi(SampleA, I) = *(u32*)(TexelPtr);
		  Mi(SampleB, I) = *(u32*)(TexelPtr + BITMAP_BYTES_PER_PIXEL);
		  Mi(SampleC, I) = *(u32*)(TexelPtr + Texture->Pitch);
		  Mi(SampleD, I) = *(u32*)(TexelPtr + Texture->Pitch + BITMAP_BYTES_PER_PIXEL);
		}
...
#define Sum(L, A) (L* (r32)A); Total += (L* (r32)A)
	    ...
	    r32 mm_add_ps       = Sum(Half, Counts.mm_add_ps);
	    r32 mm_mul_ps       = Sum(Half, Counts.mm_mul_ps);
	    r32 mm_castps_si128 = Sum(0, Counts.mm_castps_si128); 
	    r32 mm_and_ps       = Sum(Thrd, Counts.mm_and_ps);
	    r32 mm_or_si128     = Sum(Thrd, Counts.mm_or_si128);
	    r32 mm_andnot_si128 = Sum(Thrd, Counts.mm_andnot_si128);
	    r32 mm_min_ps       = Sum(Half, Counts.mm_min_ps);
	    r32 mm_max_ps       = Sum(Half, Counts.mm_max_ps);
	    r32 mm_cmple_ps     = Sum(Half, Counts.mm_cmple_ps);
	    r32 mm_cmpge_ps     = Sum(Half, Counts.mm_cmpge_ps);
	    ...
	    r32 mm_sub_ps       = Sum(Half, Counts.mm_sub_ps);
	    r32 mm_and_si128    = Sum(Thrd, Counts.mm_and_si128);
	    r32 mm_srli_epi32   = Sum(1, Counts.mm_srli_epi32);
	    r32 mm_slli_epi32   = Sum(1, Counts.mm_slli_epi32);
	    r32 mm_sqrt_ps      = Sum(3, Counts.mm_sqrt_ps);
#endif
